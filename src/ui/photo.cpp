    ...
    order_item(NULL), hide_avatar(false)
{
    ...
    equips << &weapon << &armor << &defensive_horse << &offensive_horse << &relic; //modify by ce add
    int i;
    for(i=0; i<4; i++){
...
void Photo::installEquip(CardItem *equip){
    ...
    switch(equip_card->location()){
        ...
        case EquipCard::HorseLocation: {
            const Horse *horse = qobject_cast<const Horse*>(equip_card);
        ...
        case EquipCard::RelicLocation: relic = equip; index = 3; break;
    }
...
void Photo::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget){
    ...
    // drawEquip(painter, defensive_horse, 3);
    //modify by ce 
    drawEquip(painter, offensive_horse, 2);
    drawEquip(painter, relic, 3);
