...
void RoomScene::changeTextEditBackground(){
    chat_box->setStyleSheet("background-color: rgba(0,0,0,20%);");
    log_box->setStyleSheet("background-color: rgba(0,0,0,20%);");
}
...
void RoomScene::changeHp(const QString &who, int delta, DamageStruct::Nature nature){
    ...
    if(delta < 0){
        ...
        if(nature == DamageStruct::Fire)
            ...
            doAnimation("thunder", QStringList() << who);
        else if(nature == DamageStruct::Poison)
            doAnimation("poison", QStringList() << who);
        else if(nature == DamageStruct::Air)
            doAnimation("air", QStringList() << who);
    ...
    :QDialog(parent)
{
    ...
    //modify by ce add
    damage_nature->addItem(tr("Air"), "A");
    damage_nature->addItem(tr("Poison"), "P");
    damage_nature->addItem(tr("Ice"), "I");
...
void RoomScene::doSuperAnimation(const QString &name, const QStringList &args) {
    ...
    Pixmap *item = new Pixmap(QString("image/skill/%1.png").arg(name));
    addItem(item);
...
void RoomScene::doAnimation(const QString &name, const QStringList &args){
    ...
    if(map.isEmpty()){
        ...
        map["ice"] = &RoomScene::doAppearingAnimation;
        map["air"] = &RoomScene::doAppearingAnimation;
        map["poison"] = &RoomScene::doAppearingAnimation;
        map["thunder"] = &RoomScene::doAppearingAnimation;
        // map["typhoon"] = &RoomScene::doAppearingAnimation;
        map["bang"] = &RoomScene::doAppearingAnimation;
        map["dodge"] = &RoomScene::doAppearingAnimation;
        ...
        map["huanying"] = &RoomScene::doSuperAnimation;
        ...
        map["mofu"] = &RoomScene::doSuperAnimation;
        map["baoyu"] = &RoomScene::doSuperAnimation;
        map["yuannian"] = &RoomScene::doSuperAnimation;
    }
