        ...
        QString arg2)
{
    ...
    if(!from_general.isEmpty()){
        ...
        // from = bold(from, Qt::green); modify by ce
        from = bold(from, QColor(0,255,255));
    }
    ...
    if(!tos.isEmpty()){
        ...
        // to = bold(to, Qt::red); modify by ce
        to = bold(to, QColor(153,255,0));
    }
    ...
    log = QString("<font color='%2'>%1</font>").arg(log).arg(Config.TextEditColor.name()); //modify by ce
    // log = QString("<b color='%2'>%1</b>").arg(log).arg("yellow");
