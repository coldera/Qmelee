...
lua_State *Engine::createLuaState(bool load_ai, QString &error_msg){
    ...
    if(load_ai){
        error = luaL_dofile(L, "lua/ai/melee-ai.lua");
        if(error){
            error_msg = lua_tostring(L, -1);
            return NULL;
        }
...
QStringList Engine::getKingdoms() const{
    ...
    if(kingdoms.isEmpty())
        // kingdoms << "wei" << "shu" << "wu" << "qun" << "god"; //modify by ce
        kingdoms  << "god" << "nu" << "qi" << "ling" << "kuang" << "yuan";
...
QColor Engine::getKingdomColor(const QString &kingdom) const{
    ...
    if(color_map.isEmpty()){
        color_map["ling"] = QColor(0x54, 0x79, 0x98);
        color_map["nu"] = QColor(0xD0, 0x79, 0x6C);
        color_map["qi"] = QColor(0x4D, 0xB8, 0x73);
        color_map["yuan"] = QColor(0x8A, 0x80, 0x7A);
        color_map["kuang"] = QColor(0x96, 0x94, 0x3D);
    }
