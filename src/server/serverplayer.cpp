...
//modify by ce
void ServerPlayer::updateMp(int point) {
    int mp = getMp();
    QString kingdom = getKingdom();

    this->getRoom()->setPlayerProperty(this, "mp", qMax(0, mp+point));
    
    if(point>=0) {
        gainMark(QString("@%1").arg(kingdom) , point);
    }else if(mp+point>=0){
        loseMark(QString("@%1").arg(kingdom), -point);
    }    
}
