...
bool TrustAI::useCard(const Card *card){
    if(card->inherits("Peach"))
        ...
        // case EquipCard::DefensiveHorseLocation: return !self->getDefensiveHorse();
         //modify by ce
        case EquipCard::HorseLocation: return !self->getHorse();
        case EquipCard::RelicLocation: return !self->getRelic();
        default:
            return true;
        }

    // }else if(card->inherits("ExNihilo")) //modify by ce
    }else if(card->inherits("NothingIsSomething"))
        return true;
    else
        return false;
}
