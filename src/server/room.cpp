...
//modify by ce
QList<ServerPlayer *> Room::getServerPlayers() const{
    return players;
}

//modify by ce
void Room::allSkillForbid(ServerPlayer *player) {
    QList<const Skill *> allskills = player->getVisibleSkillList();
    foreach(const Skill *skill, allskills){
        if(!skill->canForbid())
            continue;
    ...
    detachSpecialSkill(player);
    
    player->gainMark("@skill_forbid");
    
    resetAI(player);
}

void Room::allSkillRelease(ServerPlayer *player) {
    getThread()->addPlayerSkills(player, true);
    
    QList<const Skill *> allskills = player->getGeneral()->getVisibleSkillList();
    foreach(const Skill *skill, allskills){
        if(!skill->canForbid())
            continue;
    ...
    player->loseMark("@skill_forbid");

    resetAI(player);
}

void Room::detachSpecialSkill(ServerPlayer *player) {
    if(player->hasSkill("shenjing")) {
...
void Room::slashEffect(const SlashEffectStruct &effect){
    ...
    broadcastInvoke("animate", "bang:"+effect.from->objectName());
...
void Room::slashResult(const SlashEffectStruct &effect, const Card *jink){
    ...
    if(jink == NULL)
        ...
        broadcastInvoke("animate", "dodge:"+effect.to->objectName());
        thread->trigger(SlashMissed, effect.from, data);
    }
...
bool Room::isCanceled(const CardEffectStruct &effect){
    //modify by ce
    if(!effect.card->isCancelable(effect) || (effect.from && effect.from->hasFlag("can_not_unassailable")))
        return false;
    ...
    foreach(ServerPlayer *player, players){
        ...
        if(ai){
            ...
            if(positive)
                ...
                //ask_str = QString("nullification:.->%1").arg(to->objectName()); modify by ce
                ask_str = QString("unassailable:.->%1").arg(to->objectName());
...
void Room::chooseGenerals(){
    ...
    //modify by ce
    lord_list = Sanguosha->getRandomGenerals(Config.value("MaxChoice", 5).toInt()+2);
...
void Room::loseMaxHp(ServerPlayer *victim, int lose){
    ...
    //modify by ce
    setPlayerProperty(victim, "hp", qMin(victim->getHp(), victim->getMaxHP()));
...
void Room::applyDamage(ServerPlayer *victim, const DamageStruct &damage){
    ...
    //modify by ce
    case DamageStruct::Air: change_str.append("A"); break;
...
void Room::sendDamageLog(const DamageStruct &data){
    ...
    //modify by ce
    case DamageStruct::Air: log.arg2 = "air_nature"; break;
...
Card::Suit Room::askForSuit(ServerPlayer *player, const QString &reason){
    AI *ai = player->getAI();
    if(ai)
        return ai->askForSuit(reason); //modify by ce add param reason
...
void Room::makeCheat(const QString &cheat_str){
    // QRegExp damage_rx(":(.+)->(\\w+):([NTFRL])(\\d+)"); modify by ce
...
void Room::makeDamage(const QStringList &texts){
    ...
    //modify by ce
    case 'A': damage.nature = DamageStruct::Air; break;
    case 'P': damage.nature = DamageStruct::Poison; break;
    case 'I': damage.nature = DamageStruct::Ice; break;
